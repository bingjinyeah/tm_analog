    ...
    *(p + 1) = (*pframe).Data[2];
    ...
    if((quantity >= 1)&&(quantity <= MB_COILS_AMOUNT))    
    {
        ...
        {   
            ...
            {
                if (MB_DRIVER_GetCoils(start_addr + i)) //线圈状态为1
                {
    ...
    *(p + 1) = (*pframe).Data[2];
    ...
    if((quantity >= 1)&&(quantity <= MB_DI_AMOUNT))    
    {
        ...
        {   
            ...
            {
                if (MB_DRIVER_GetDisIn(start_addr + i)) //离散量输入状态为1
                {
                    switch(i % 8)                //一个字节中的某一位
                    {
                        case 0: reply_frame.Data[i/8 + 1] |= 0x01; break;
...
//返回该地址线圈的状态  0或1    
uint8_t MB_DRIVER_GetCoils(uint16_t address)
{
    uint16_t offset;
    uint8_t  b,c;
    offset = address / 8;
...
//返回该地址离散量输入的状态  0或1    
uint8_t MB_DRIVER_GetDisIn(uint16_t address)
{
    uint16_t offset;
    uint8_t  b,c;
    offset = address / 8;
...
//写该线圈地址下单个线圈值（写单个位的输出值） 
void MB_DRIVER_SetCoils(uint16_t address,uint16_t data)      //data为0 或 1   
{
    if (data)
    {
        //置1
